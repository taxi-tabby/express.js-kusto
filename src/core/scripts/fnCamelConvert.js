...
function analyzeWordType(word) {
    ...
    const maxConsonantCluster =
        consecutiveConsonants.length > 0
    ...
    if (vowelRatio >= 0.3)
        pronounceabilityScore += 3; // Good vowel ratio
    else if (vowelRatio >= 0.2) pronounceabilityScore += 1; // Acceptable vowel ratio

    // Consonant cluster penalty
    if (maxConsonantCluster >= 4)
        pronounceabilityScore -= 3; // Very hard to pronounce
    else if (maxConsonantCluster >= 3)
        pronounceabilityScore -= 2; // Hard to pronounce
