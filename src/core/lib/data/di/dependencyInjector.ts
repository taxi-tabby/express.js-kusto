...
export function pathToCamelCaseIdentifier(filePath: string): string {
    const withoutExt = filePath.replace(
        /\.module\.ts$|\.middleware\.ts$|\.middleware\.interface\.ts$/,
        '',
    );
...
export class DependencyInjector {
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            log.Info(
                `Dependency injection initialized with ${Object.keys(this.modules).length} modules and ${Object.keys(this.middlewares).length} middlewares`,
            );
        } catch (error) {
        ...
        for (const moduleName of moduleNames) {
            try {
                ...
                if (typeof ModuleClass === 'function') {
                    ...
                    log.Warn(
                        `Module ${moduleName} resolved to unexpected type: ${typeof ModuleClass}`,
                    );
                    this.modules[moduleName] = ModuleClass;
                }
    ...
    private async loadMiddlewares(): Promise<void> {
        ...
        for (const middlewareName of middlewareNames) {
            try {
                ...
                const MiddlewareFunction = this.resolveMiddlewareFunction(
                    middlewareExports,
                    middlewareName,
                );
                ...
                if (typeof MiddlewareFunction === 'function') {
                    ...
                    log.Warn(
                        `Middleware ${middlewareName} resolved to unexpected type: ${typeof MiddlewareFunction}`,
                    );
                    this.middlewares[middlewareName] = MiddlewareFunction;
                }
        ...
        log.Info(
            `Middleware loading complete. Loaded middlewares: ${Object.keys(this.middlewares).join(', ')}`,
        );
    }
    ...
    private resolveModuleClass(moduleExports: any, moduleName: string): any {
        ...
        if (typeof moduleExports === 'object' && moduleExports !== null) {
            ...
            const classLikeKey = functionKeys.find(
