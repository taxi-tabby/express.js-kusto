...
describe('CrudQueryParser.validateIncludes', () => {
    it('policy 가 undefined 일 때 어떤 검증도 하지 않는다', () => {
        expect(() => CrudQueryParser.validateIncludes(['a.b.c.d.e'], undefined)).not.toThrow();
    });
    ...
    it('maxDepth 가 지정됐고 점 깊이가 초과할 때 INCLUDE_DEPTH_EXCEEDED 를 throw 한다', () => {
        expect(() => CrudQueryParser.validateIncludes(['a.b.c.d'], { maxDepth: 3 })).toThrow(
            expect.objectContaining({ code: 'INCLUDE_DEPTH_EXCEEDED', statusCode: 400 }),
        );
    });

    it('항목 점 깊이가 maxDepth 와 같을 때 통과한다 (경계값)', () => {
        expect(() => CrudQueryParser.validateIncludes(['a.b.c'], { maxDepth: 3 })).not.toThrow();
    });

    it('allowed 에 정확히 일치하는 path 가 들어올 때 통과한다', () => {
        expect(() =>
            CrudQueryParser.validateIncludes(['author'], {
