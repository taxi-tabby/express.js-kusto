...
| 옵션 | 동작 |
|------|------|
| `--dry-run` | 생성/갱신/삭제 계획만 출력하고 **어떤 파일도 쓰지 않음** |
...
- **자동 백업 + 롤백**: 적용 전 변경/삭제 대상 파일을 백업하고, 적용 중 오류가 나면 자동 원복한다. 원복이 불완전하면 백업을 보존하고 경로를 안내한다. (과거의 "자동 백업/롤백 없음" 경고는 더 이상 유효하지 않다.)
