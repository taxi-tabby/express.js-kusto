...
확장이 산출하는 빌드 에셋(예: 클라이언트 번들)은 관례적으로 프로젝트 루트의 **`.kusto/`** 디렉터리에 생성됩니다 — Next 의 `.next` 와 동일 성격의 **내부 생성 공용 디렉터리**로, 빌드 타임에만 만들어지고 버전 관리에서 제외됩니다(`.gitignore`). 런타임에는 있으면 정적 서빙 등에 쓰이고, 확장이 없으면 생성되지 않으며 그 상태로도 정상 동작합니다(예: 풀스택 React 확장의 클라이언트 번들은 `.kusto/react`).
