...
function buildDatabaseTypesContent(dbFolders) {
    ...
    const instanceTypes = dbFolders
        .map((dbName) => `type ${capitalize(dbName)}Instance = ${capitalize(dbName)}PrismaClient;`)
        .join('\n');

    // Generate DatabaseClientMap interface
    const clientMapEntries = dbFolders
        .map((dbName) => `  ${dbName}: ${capitalize(dbName)}Instance;`)
        .join('\n');
    ...
    const methodOverloads = dbFolders
        .map((dbName) => `  getWrap(databaseName: '${dbName}'): ${capitalize(dbName)}Instance;`)
        .join('\n');

    const getClientOverloads = dbFolders
        .map(
            (dbName) =>
