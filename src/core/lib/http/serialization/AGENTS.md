...
- **Inbound**: `@lib/http/validation/requestHandler` consumes `ResponseSerializer`/`applyResponseSerializer`, `@lib/http/routing/expressRouter` consumes `serialize`/`serializeBigInt`/`ResponseSerializer`/`applyResponseSerializer`, and `@lib/crud/crudResponseSerializer` reuses `applyResponseSerializer` for CRUD response filtering. `serializationMiddleware` is an opt-in middleware that is not included in the default global policy stack (`defaultGlobalMiddleware()`); when needed, the app registers it directly in `src/app/routes/middleware.ts` or similar.
