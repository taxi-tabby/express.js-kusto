...
- **Initialization order** (`initialize()`): `initializePrismaManager()` → `initializeRepositoryManager()` → `initializeDependencyInjector()` → `setupExpress()` → `setupHealthCheck()` → `setupDocumentationRoutes()` → `loadRoutes()` → `setupViews()` → Schema API registration.
