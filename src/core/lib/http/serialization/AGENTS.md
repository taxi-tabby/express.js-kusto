...
- **Inbound**: `@lib/http/validation/requestHandler` consumes `ResponseSerializer`/`applyResponseSerializer`, and `@lib/http/routing/expressRouter` consumes `serialize`/`serializeBigInt`/`ResponseSerializer`/`applyResponseSerializer`. `@lib/crud/crudResponseSerializer` (CRUD response filtering reuses `applyResponseSerializer`). `serializationMiddleware` is an opt-in middleware that is not included in the default global policy stack (`defaultGlobalMiddleware()`); when needed, the app registers it directly in `src/app/routes/middleware.ts` or similar.
