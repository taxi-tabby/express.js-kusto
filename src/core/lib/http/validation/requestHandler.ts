...
import { ResponseSerializer, applyResponseSerializer } from '@lib/http/serialization/serializer';
...
export type ExtractFieldType<T extends FieldSchema> = T['type'] extends 'string'
    ? string
...
export class RequestHandler {
        ...
        return (req: ValidatedRequest, res: Response, next: NextFunction) => {
            ...
            if (config.params) {
                ...
                if (!paramsResult.isValid) {
                    errors.push(
                        ...paramsResult.errors.map((err) => ({ ...err, source: 'params' })),
                    );
                } else {
    ...
    static validateAndFilterResponse(data: any, schema: Schema): any {
        ...
        if (undefinedFields.length > 0) {
            throw new Error(
                `Response contains undefined fields not in schema: ${undefinedFields.join(', ')}`,
            );
        }
    ...
    static sendSuccess(
        ...
        if (responseConfig && !responseConfig[statusCode]) {
            ...
            return this.sendError(
                res,
                500,
                `Internal server error - status code ${statusCode} not defined in response config`,
            );
        }
    ...
    static validateHandlerImplementation(
        ...
        for (const code of nonDefaultStatusCodes) {
            ...
            const entryMeta = responseConfig[code as unknown as number] as
                | (Schema & ResponseConfigEntryMeta)
                | undefined;
            ...
            if (
                !statusSetPattern.test(handlerWithoutComments) &&
                !errorHandlerPattern.test(handlerWithoutComments)
            ) {
    ...
    static createHandler(
        ...
        if (config.response && process.env.NODE_ENV !== 'production') {
            try {
                ...
                const missingImplementations = this.validateHandlerImplementation(
                    config.response,
                    handlerSource,
                    config.sourceInfo,
                );
                ...
                if (
                    ...
                    process.env.STRICT_STATUS_CODE_CHECK === 'true'
                ) {
