...
export class PrismaSchemaAnalyzer {
    ...
    public static getInstance(
        prismaClient: PrismaClientAny,
        databaseName: string = 'default',
    ): PrismaSchemaAnalyzer {
        if (!PrismaSchemaAnalyzer.instances.has(databaseName)) {
            PrismaSchemaAnalyzer.instances.set(
                databaseName,
                new PrismaSchemaAnalyzer(prismaClient, databaseName),
            );
        }
        ...
        try {
            ...
            if ((this.prismaClient as any)._runtimeDataModel) {
                ...
                if (runtimeDataModel && runtimeDataModel.models) {
                    ...
                    const models: DmmfModelShape[] = Object.entries(runtimeDataModel.models).map(
            ...
            if (!dmmf) {
                ...
                log.Debug(
                    'Client properties:',
                    Object.keys(this.prismaClient as any).filter(
                        (k) => k.includes('dmmf') || k.includes('DMMF'),
                    ),
                );
                ...
                if ((this.prismaClient as any).Prisma) {
                    log.Debug(
                        'Prisma namespace properties:',
                        Object.keys((this.prismaClient as any).Prisma).filter(
                            (k) => k.includes('dmmf') || k.includes('DMMF'),
                        ),
                    );
                }
                ...
                if (constructor) {
                    log.Debug(
                        ...
                        Object.keys(constructor).filter(
                            (k) => k.includes('dmmf') || k.includes('DMMF'),
                        ),
                    );
                    log.Debug(
                        'Constructor static properties:',
                        Object.getOwnPropertyNames(constructor).filter(
                            (k) => k.includes('dmmf') || k.includes('DMMF'),
                        ),
                    );
                }
    ...
    public getJsonFields(modelName: string): string[] {
        ...
        return model.fields.filter((field) => field.type === 'Json').map((field) => field.name);
    }
    ...
    public getEnumValues(type: string): string[] | undefined {
        ...
        if (this.loadedEnums[type] && typeof this.loadedEnums[type] === 'object') {
            ...
            if (enumObj.values) {
                return Array.isArray(enumObj.values)
                    ? enumObj.values
                    : Object.values(enumObj.values);
            }
