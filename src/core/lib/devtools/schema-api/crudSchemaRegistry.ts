...
export class CrudSchemaRegistry {
    ...
    private autoRegisterModel(
        ...
        try {
            ...
            // 소프트 삭제 필드 확인
            const softDeleteField = modelInfo.fields.find(
                (field) => field.name === 'deletedAt' || field.name === 'deleted_at',
            );
    ...
    public registerSchema(
        ...
        try {
            ...
            if (!modelInfo) {
                log.Warn(
                    `Model '${modelName}' not found in ${analyzer.getDatabaseName()}, skipping schema registration`,
                );
                return;
            }
            ...
            if (options.only) {
                ...
                enabledActions = defaultActions.filter(
    ...
    public getSchemasByDatabase(databaseName: string): SchemaApiResponse<CrudSchemaInfo[]> {
        ...
        const schemas = Array.from(this.schemas.values()).filter(
            (schema) => schema.databaseName === databaseName,
        );
    ...
    public getTypeOrmCompatibleSchema(databaseName?: string, modelName?: string): any {
        ...
        // 데이터베이스별 통계
        const databaseStats = schemas.reduce(
            (stats, schema) => {
    ...
    private generateCrudInfo(schema: CrudSchemaInfo): any {
        ...
        return {
            ...
            // 사용 가능한 엔드포인트 생성
            availableEndpoints: this.buildEndpointsForActions(
                enabledActions,
                basePath,
                schema.primaryKey,
                options,
    ...
    private collectAllowedFilters(model: CrudSchemaInfo['model']): string[] {
        return model.fields
            .filter((field) => field.jsType === 'string' && !field.relationName && !field.isId)
            .slice(0, 5) // 최대 5개만
    ...
    private convertRelationsToTypeOrmFormat(relations: any[], modelName: string): any[] {
        ...
        for (const relation of relations) {
            ...
            if (this.relationshipManager.isManyToManyRelation(relation, modelName)) {
                const manyToManyConfig = this.relationshipManager.getManyToManyConfig(
                    relation,
                    modelName,
                );
                if (manyToManyConfig) {
                    ...
                    log.Warn(
                        `Many-to-Many config failed: ${modelName}.${relation.name} -> ${relation.model}`,
                    );
                }
            }
            // 일반 관계들 처리
                ...
                if (!this.relationshipManager.isIntermediateTableRelation(relation, modelName)) {
                    const convertedRelation = this.convertRelationToTypeOrmRelation(
                        relation,
                        modelName,
                    );
    ...
    private isEnumType(type: string): boolean {
        ...
        return (
            !PRISMA_SCALAR_TYPES.includes(type) && type.charAt(0).toUpperCase() === type.charAt(0)
        );
    }
    ...
    public getAutoRegisteredSchemas(): SchemaApiResponse<CrudSchemaInfo[]> {
        ...
        const autoRegisteredSchemas = Array.from(this.schemas.values()).filter(
            (schema) => schema.isAutoRegistered,
        );
    ...
    public getManualRegisteredSchemas(): SchemaApiResponse<CrudSchemaInfo[]> {
        ...
        const manualRegisteredSchemas = Array.from(this.schemas.values()).filter(
            (schema) => !schema.isAutoRegistered,
        );
