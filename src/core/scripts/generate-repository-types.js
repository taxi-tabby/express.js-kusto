...
function generateRepositoryTypes() {
    ...
    const repositoryTypes = repositories
        .map((repository) => {
    ...
    const repositoryTypeMap = repositories
        .map((repository) => `  '${repository.propertyName}': ${repository.className}Type;`)
        .join('\n');
...
export interface RepositoryTypeMap {
  ...
  // Add TypeScript files ending with .repository.ts to src/app/repos/ and regenerate types
    ...
    const repositoryRegistry = repositories
        .map(
            (repository) =>
