...
- **Key exports**: `default loadExtensions(dir = './src/app/extensions'): KustoExtension[]` — validates each activation's default export with `isKustoExtension`, registers its `routerMethods` on `ExpressRouter` immediately (must precede route loading), collects it in the registry, and returns the loaded list. Activations are processed in filename order; skips `.d.ts`, `index`, `AGENTS`. Also exports `isExtensionFile(name)` (the activation-file predicate) and `bundledExtensionsToDiscovered(mod)` (the pure build-branch mapping).
- **Dual discovery (mirrors routes)**: in **dev** (`WEBPACK_BUILD !== 'true'`) it scans `src/app/extensions/*.ts`/`*.js` at runtime and `require`s each (no-op `[]` when the folder is absent). In the **webpack build** (`WEBPACK_BUILD === 'true'`) the raw activation files are NOT bundled, so a runtime fs scan + `require` fails; instead it reads the bundled `@core/tmp/extensions-map`. That map is codegen'd at build time by `src/core/scripts/generate-extensions-map.js` (static default imports of every activation, plus a parallel `extensionSources` array used for diagnostic labels → webpack bundles them), wired into `generate.js`'s build scripts. Without this, extension methods (e.g. `GET_REACT`) are never registered in the build and routes that call them fail with "X is not a function". The build-branch mapping (`{ extensions, extensionSources }` → discovered list, with index fallback) is the pure `bundledExtensionsToDiscovered`, unit-tested by `tests/unit/extensions/bundled-extensions.test.ts`; the codegen path transform by `generate-extensions-map.test.ts`; the full build→serve wiring is covered by manual E2E.
- **SSOT note**: `isExtensionFile` is necessarily duplicated in the JS codegen (`.js` build script can't import the `.ts` runtime module); dev and build MUST classify the same files, so `tests/unit/extensions/extension-file-filter-parity.test.ts` guards their equivalence.
