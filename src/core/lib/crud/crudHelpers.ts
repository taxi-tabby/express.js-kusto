...
export class CrudQueryParser {
        ...
        Object.keys(query).forEach((key) => {
            ...
            if (match) {
                ...
                if (typeof fieldValue === 'string' && fieldValue.length > 0) {
                    fields[resourceType] = fieldValue
                        .split(',')
        ...
        if (query.filter && typeof query.filter === 'object') {
            Object.entries(query.filter).forEach(([filterExpression, value]) => {
                ...
                if (filterExpression.toLowerCase() === 'or' && typeof value === 'object') {
                    ...
                    const parsed = this.parseFilterExpression(
                        filterExpression,
                        value,
                        modelName,
                        schemaAnalyzer,
     ...
     */
    private static parseOrConditions(
        orObject: any,
        modelName?: string,
        schemaAnalyzer?: any,
    ): any[] {
    ...
    private static parseFilterExpression(
        ...
        for (const op of operators) {
            if (expression.endsWith('_' + op)) {
                ...
                const parsedValue = this.parseFilterValue(
     ...
     */
    private static smartTypeConversion(
        value: any,
        fieldName?: string,
        modelName?: string,
        schemaAnalyzer?: any,
     ...
     */
    private static getFieldTypeFromSchema(
        fieldName: string,
        modelName: string,
        schemaAnalyzer: any,
    ): { type: string; nativeType?: string } | null {
        try {
    ...
    private static convertValueByType(
        value: string,
        fieldTypeInfo: { type: string; nativeType?: string },
    ): any {
...
export class PrismaQueryBuilder {
            ...
            if (field === '_or' && Array.isArray(conditions)) {
                conditions.forEach((orCondition) => {
                    ...
                    Object.entries(orCondition).forEach(([orField, orConditions]) => {
                        ...
                        if (orField.includes('.')) {
                            this.buildNestedWhereCondition(
                                orWhere,
                                orField,
     ...
     */
    private static buildNestedWhereCondition(
        where: any,
        fieldPath: string,
...
export class CrudResponseFormatter {
    ...
    static createPaginationMeta(
        ...
        const metadata: any = {
            ...
            ...(finalIncludedRelations &&
                finalIncludedRelations.length > 0 && {
    ...
    private static applyLibrarySpecificSanitizers(message: string): string {
        const sanitizers = [this.sanitizePrismaSpecificErrors];
...
export class JsonApiTransformer {
     ...
     */
    static inferResourceTypeFromData(
        data: any,
        relationshipName: string,
        isArray: boolean,
    ): string {
        ...
        if (isArray) {
            relationData.forEach((relItem) => {
                ...
                if (!processedResources.has(resourceKey)) {
                    ...
                    included.push(
                        this.transformToResource(relItem, {
                            resourceType,
                            primaryKey: 'id',
                            fields: resourceFields,
                            baseUrl,
            ...
            if (!processedResources.has(resourceKey)) {
                processedResources.add(resourceKey);
                included.push(
                    this.transformToResource(relationData, {
                        resourceType,
                        primaryKey: 'id',
                        fields: resourceFields,
                        baseUrl,
