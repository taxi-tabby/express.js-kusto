...
export class ErrorHandler {
    ...
    static applySecurity(
        ...
        const isDevelopment =
            securityOptions?.isDevelopment ?? process.env.NODE_ENV === 'development';
        ...
        if (!shouldSanitize) {
            ...
            return {
                ...
                stack:
                    typeof normalizedError.stack === 'string'
     ...
     * (OPERATION_NOT_ALLOWED / CONNECTION_ERROR)를 보존하기 위해 override 로 처리한다.
     */
    private static mapPrismaErrorCodes(code: string): string {
        // per-consumer override: 정규 맵과 결과가 갈리는 코드 (behavior-preserving)
        if (code === 'P2030') return ERROR_CODES.OPERATION_NOT_ALLOWED;
        if (code === 'P2031') return PRISMA_ERROR_CODES.CONNECTION_ERROR;
     ...
     * CRUD 형식 에러 응답 생성
     */
    private static formatCrudError(normalizedError: NormalizedError, context?: any): any {
        return {
            error: {
