...
export class PrismaManager implements PrismaManagerWrapOverloads, PrismaManagerClientOverloads {
        ...
        try {
            ...
            if (isWebpackBuild) {
                ...
                try {
                    ...
                    log.Error(
                        `Failed to load Prisma client from dist for ${folderName}:`,
                        requireError,
                    );
                ...
                const clientPath = path.join(folderPath, 'client');
                log.Debug(
                    `Loading Prisma client for ${folderName} from development path: ${clientPath}`,
                );
                ...
                try {
                    ...
                    if (process.env.NODE_ENV === 'development') {
                        ...
                        Object.keys(require.cache).forEach((key) => {
                            ...
                            if (
                                normalizedKey.includes(normalizedClientPath) ||
                    ...
                    if (!fs.existsSync(clientIndexPath)) {
                        throw new Error(
                            `Prisma client index.js not found at: ${clientIndexPath}. Please run 'npx prisma generate --schema=${path.join(folderPath, 'schema.prisma')}'`,
                        );
                    }

                    if (!fs.existsSync(clientIndexTsPath)) {
                        log.Warn(
                            `Prisma client TypeScript definitions not found at: ${clientIndexTsPath}`,
                        );
                    }
                    ...
                    if (!DatabasePrismaClient) {
                        throw new Error(
                            `PrismaClient not found in module: ${importPath}. Module exports: ${Object.keys(clientModule || {}).join(', ')}`,
                        );
                    }
                    ...
                    if (typeof DatabasePrismaClient !== 'function') {
                        throw new Error(
                            `PrismaClient is not a constructor function. Type: ${typeof DatabasePrismaClient}`,
                        );
                    }

                    log.Debug(
                        `Successfully loaded Prisma client for ${folderName} from development path`,
                    );
                } catch (importError: any) {
                    log.Error(
                        `Failed to load Prisma client from development path for ${folderName}:`,
                        importError,
                    );
                    ...
                    if (fs.existsSync(distClientIndexPath)) {
                        ...
                        try {
                            ...
                            if (!DatabasePrismaClient) {
                                throw new Error(
                                    `PrismaClient not found in dist module: ${distClientIndexPath}`,
                                );
                            }
                            ...
                            throw new Error(
                                `Both development and dist client loading failed for ${folderName}. Development error: ${importError.message}, Dist error: ${distError}`,
                            );
                        }
                    } else {
                        throw new Error(
                            `Development client loading failed for ${folderName}: ${importError.message}. Dist fallback not available.`,
                        );
                    }
                }
            }
    ...
    private async checkIfGenerated(folderName: string): Promise<boolean> {
        try {
            ...
            const hasGenerator =
                /generator\s+\w+\s*{[\s\S]*?provider\s*=\s*["']prisma-client-js["'][\s\S]*?}/m.test(
                    schemaContent,
                );
    ...
    private getSchemaProvider(folderName: string): string {
        try {
            ...
            log.Warn(
                `Failed to parse schema.prisma provider, falling back to 'postgresql': ${folderName}`,
     ...
     */
    private async createDriverAdapter(
        folderName: string,
        ...
        switch (provider) {
            ...
            default:
                log.Warn(
                    `Unknown provider '${provider}' for ${folderName}, attempting connection without adapter`,
                );
                return null;
        }
    }
    ...
    private getDatabaseUrl(folderName: string): string {
        try {
            ...
            if (!url) {
                throw new Error(
                    `Environment variable ${envVarName} not found for database ${folderName}`,
                );
            }
            ...
            throw new Error(
                `Failed to get database URL for ${folderName}: ${error instanceof Error ? error.message : 'Unknown error'}`,
            );
        }
    }
    ...
    private getDatasourceName(folderName: string): string {
        try {
            ...
            throw new Error(
                `Failed to get datasource name for ${folderName}: ${error instanceof Error ? error.message : 'Unknown error'}`,
            );
        }
    }
    ...
    public async reconnectDatabase(databaseName: string): Promise<boolean> {
        ...
        if (now < cooldownUntil) {
            ...
            log.Warn(
                `Database '${databaseName}' reconnection in cooldown (${remainingSec}s remaining)`,
            );
            return false;
        }
        ...
        if (attempts >= this.MAX_RECONNECTION_ATTEMPTS) {
            log.Error(
                `Max reconnection attempts (${this.MAX_RECONNECTION_ATTEMPTS}) reached for database '${databaseName}', cooldown ${this.RECONNECTION_COOLDOWN_MS / 1000}s`,
            );
        ...
        try {
            ...
            if (existingClient) {
                try {
                    ...
                    log.Debug(
                        `Existing client disconnect failed before reconnect: ${databaseName}`,
            ...
            log.Error(
                `Failed to reconnect to database '${databaseName}' (attempt ${attempts + 1}/${this.MAX_RECONNECTION_ATTEMPTS}):`,
        ...
        while (connectionAttempts < maxAttempts) {
            try {
                ...
                if (connectionAttempts >= maxAttempts) {
                    log.Error(
                        `Connection failed for ${databaseName} after ${maxAttempts} attempts:`,
                        connectError,
                    );
                    throw connectError;
                }
    ...
    private async recreateClient(databaseName: string): Promise<void> {
        ...
        if (!config || !config.isGenerated) {
            throw new Error(
                `Cannot recreate client for '${databaseName}': config not found or not generated`,
            );
        }
    ...
    public async getClient<T = any>(databaseName: string): Promise<T> {
        try {
            ...
            if (!this.initialized) {
                ...
                log.Error(
                    `Called from: ${callerInfo.filePath}${callerInfo.lineNumber ? `:${callerInfo.lineNumber}` : ''}`,
                );
                throw new Error(
                    '데이터베이스 관리자가 초기화되지 않았습니다. 애플리케이션 시작 시 initialize()를 호출했는지 확인하세요.',
                );
            }
            ...
            if (!this.configs.has(databaseName)) {
                ...
                throw new Error(
                    `데이터베이스 '${databaseName}'를 찾을 수 없습니다. 사용 가능한 데이터베이스: ${dbList}`,
                );
            }
                    ...
                    for (let attempt = 0; attempt <= maxRetries; attempt++) {
                        try {
                            ...
                            if (manager.isConnectionError(error) && attempt < maxRetries) {
                                ...
                                log.Debug(
                                    `DB connection error detected (${error.message?.substring(0, 50)}...), retrying in ${delay / 1000}s... (${attempt + 1}/${maxRetries})`,
                                );
                                ...
                                try {
                                    ...
                                    log.Warn(
                                        `Reconnection attempt failed (will retry on next attempt): ${databaseName}`,
                                        { attempt: attempt + 1, message: reconnectError?.message },
                                    );
                                }
                                continue;
                            }
    ...
    public async disconnectAll(): Promise<void> {
        const disconnectPromises = Array.from(this.databases.values()).map((client) =>
            client
                .$disconnect()
                .catch((error: any) => log.Error('Error disconnecting Prisma client:', error)),
        );
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            if (config) {
                ...
                Object.keys(require.cache).forEach((key) => {
                    const normalizedKey = key.replace(/\\/g, '/');
                    if (
                        normalizedKey.includes(`/db/${databaseName}/`) &&
