...
describe('나머지 plain verb serialize', () => {
    it('POST {omit}', async () => {
        const app = appWith((r) =>
            r.POST(async () => ({ id: 1, password: 'x' }), { serialize: { omit: ['password'] } }),
        );
    ...
    it('PUT {pick}', async () => {
        const app = appWith((r) =>
            r.PUT(async () => ({ id: 1, a: 1, b: 2 }), { serialize: { pick: ['id', 'a'] } }),
        );
    ...
    it('PATCH 함수형', async () => {
        const app = appWith((r) =>
            r.PATCH(async () => ({ id: 1, t: 'x' }), { serialize: (u) => ({ id: u.id }) }),
        );
    ...
    it('DELETE {omit}', async () => {
        const app = appWith((r) =>
            r.DELETE(async () => ({ id: 1, internal: true }), {
                serialize: { omit: ['internal'] },
            }),
        );
    ...
    it('GET_SLUG / POST_SLUG / PUT_SLUG / DELETE_SLUG / PATCH_SLUG {omit}', async () => {
        const app = appWith((r) => {
            r.POST_SLUG(['id'], async () => ({ id: 1, password: 'x' }), {
