...
async function extractNamedExports(filePath) {
    ...
    try {
        ...
        console.warn(
            `Could not dynamically import ${filePath}, falling back to static analysis:`,
            error.message,
        );
        return extractNamedExportsStatic(filePath);
    }
...
function extractNamedExportsStatic(filePath) {
    try {
        ...
        const uniqueExports = [...new Set(namedExports)].filter(
            (name) =>
                name &&
...
async function scanDirectory(dirPath, basePath = '', modules = []) {
    ...
    for (const item of items) {
        ...
        if (item.isDirectory()) {
            ...
            await scanDirectory(itemPath, relativePath, modules);
        } else if (item.isFile() && item.name.endsWith('.module.ts')) {
            ...
            const actualInterfaceName =
                exportedInterfaces.length > 0
                    ...
                    : `${toPascalCase(cleanFileName)}MiddlewareParams`;
...
async function generateInjectableTypes() {
    ...
    console.log(
        'Found injectable modules:',
        modules.filter((m) => m.type === 'module').map((m) => m.modulePath),
    );
    console.log(
        'Found middleware modules:',
        modules.filter((m) => m.type === 'middleware').map((m) => m.modulePath),
    );
    console.log(
        'Found middleware interface modules:',
        modules.filter((m) => m.type === 'middleware-interface').map((m) => m.modulePath),
    );
    ...
    console.log(
        'Found named middleware exports:',
        namedMiddlewareEntries.map((m) => `${m.namedExport} from ${m.modulePath}`),
    ...
    const middlewareTypes = middlewareEntries
        .map((middleware) => {
    ...
    const namedMiddlewareTypes = namedMiddlewareEntries
        .map((middleware) => {
            return `type ${middleware.className}Type = typeof ${middleware.className};`;
        })
        .join('\n'); // Generate middleware parameter type definitions
    const middlewareParamTypes = middlewareInterfaceEntries
        .map((middlewareInterface) => {
            // Generate dynamic alias name based on the module path and property name
            const pathPrefix = toPascalCase(
    ...
    const allTypes = [moduleTypes, middlewareTypes, namedMiddlewareTypes, middlewareParamTypes]
        .filter((t) => t)
        .join('\n');

    // Generate Injectable interface for modules
    const injectableProperties = moduleEntries
        .map((module) => `  ${module.propertyName}: ${module.className}Type;`)
        .join('\n'); // Generate Middleware interface for middlewares
    const middlewareProperties = middlewareEntries
        .concat(namedMiddlewareEntries)
        .map((middleware) => `  ${middleware.propertyName}: ${middleware.className}Type;`)
        .join('\n');
    // Generate MiddlewareParams interface for middleware parameters
    const middlewareParamProperties = middlewareInterfaceEntries
        .map(
            (middlewareInterface) =>
...
export interface MiddlewareParams {
  ...
  // Add *.middleware.interface.ts files to src/app/injectable/ and regenerate types
    ...
    const moduleRegistry = moduleEntries
        .map(
            (module) =>
    ...
    const middlewareRegistry = middlewareEntries
        .concat(namedMiddlewareEntries)
        .map(
            (middleware) =>
    ...
    middlewareEntries.forEach((middleware) => {
        ...
        const matchingParam = middlewareInterfaceEntries.find((paramInterface) => {
            ...
            const middlewareDir = middlewareBasePath.substring(
                0,
                middlewareBasePath.lastIndexOf('/'),
            );
    ...
    const middlewareParamMappingExport =
