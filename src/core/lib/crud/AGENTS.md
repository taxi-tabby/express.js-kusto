...
**Main exports**:
- `interface CrudBuilderContext` — the set of capabilities the builder requires from ExpressRouter (`router`, `basePath`, `schemaRegistry`, `schemaAnalyzer`, `wrapHandler`, `wrapMiddleware`, `registerDocumentation`).
...
**Layering direction**:
- **Inbound**: `@lib/http/routing/expressRouter` instantiates `CrudRouteBuilder` and injects itself as the `CrudBuilderContext`. The routing/middleware/documentation tiers reference `jsonApiConstants`.
